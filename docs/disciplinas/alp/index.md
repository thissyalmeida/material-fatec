![Banner](https://github.com/thissyalmeida/material-fatec/blob/main/assets/images/banner-alp.png?raw=true)

# ⚙️ Algoritmos e Lógica de Programação

> **Curso:** Desenvolvimento de Software Multiplataforma (DSM)
> **Carga Horária:** 80 horas
> **Professora:** **Profª Me. Thissiany Almeida**

---

# Bem-vindo(a)!

Esta disciplina apresenta os fundamentos da construção de algoritmos e da lógica de programação, desenvolvendo o raciocínio lógico e a capacidade de resolver problemas utilizando a linguagem **C**.

Ao longo do semestre serão abordados conceitos essenciais para a formação do desenvolvedor de software, servindo como base para disciplinas como Estrutura de Dados, Programação Web e Inteligência Artificial.

---

## 📌 Painel da Disciplina

| Informação               | Status                |
| ------------------------ | --------------------- |
| 📅 Próxima Aula          | Consulte o Cronograma |
| 📚 Material mais recente | Em atualização        |
| 📝 Próxima Atividade     | Consulte Exercícios   |
| 🎯 Próxima Avaliação     | Consulte Avaliações   |

---

## 🚀 Acesso Rápido

<div class="grid cards" markdown>

* :material-file-document-outline: **Plano de Ensino**

  ---

  Objetivos, competências, metodologia e critérios de avaliação.

  [:material-arrow-right: Acessar](plano-de-ensino.md)

* :material-calendar-month: **Cronograma**

  ---

  Organização semanal das aulas e conteúdos.

  [:material-arrow-right: Acessar](cronograma.md)

* :material-book-open-page-variant: **Materiais**

  ---

  Slides, códigos, vídeos e materiais de apoio.

  [:material-arrow-right: Acessar](materiais.md)

* :material-pencil: **Exercícios**

  ---

  Listas de exercícios, desafios e atividades práticas.

  [:material-arrow-right: Acessar](exercicios.md)

* :material-clipboard-check-outline: **Avaliações**

  ---

  Critérios, datas e orientações das avaliações.

  [:material-arrow-right: Acessar](avaliacoes.md)

* :material-lightbulb-on-outline: **Projetos**

  ---

  Projetos práticos desenvolvidos durante o semestre.

  [:material-arrow-right: Acessar](projetos.md)

* :material-bookshelf: **Bibliografia**

  ---

  Livros e referências recomendadas.

  [:material-arrow-right: Acessar](bibliografia.md)

* :material-bullhorn-outline: **Avisos**

  ---

  Comunicados importantes da disciplina.

  [:material-arrow-right: Acessar](avisos.md)

</div>

---

## 🧠 Competências Desenvolvidas

Ao concluir esta disciplina, espera-se que o estudante seja capaz de:

* Interpretar problemas computacionais.
* Desenvolver algoritmos utilizando pseudocódigo.
* Implementar soluções utilizando a linguagem C.
* Utilizar estruturas condicionais e de repetição.
* Modularizar programas por meio de funções.
* Trabalhar com vetores e matrizes.
* Aplicar boas práticas de programação.

---

## 🔗 Recursos da Disciplina

<div class="grid cards" markdown>

* :material-github: **Repositório GitHub**

  ---

  Códigos-fonte, exemplos e materiais.

  [:material-arrow-right: GitHub](#)

* :material-web: **GitHub Pages**

  ---

  Portal da disciplina.

  [:material-arrow-right: Acessar](#)

* :material-download: **Downloads**

  ---

  Arquivos disponibilizados durante o semestre.

  [:material-arrow-right: Em breve](#)

</div>

---

## 📢 Avisos

!!! info "Bem-vindo!"

```
Este portal será atualizado continuamente durante o semestre.
```

!!! tip "Dica"

```
Consulte o cronograma antes de cada aula para verificar os materiais e atividades previstas.
```

---

## 📈 Trilha de Aprendizagem

Esta disciplina faz parte da trilha de formação em Computação.

![Trilha](https://github.com/thissyalmeida/material-fatec/blob/main/assets/images/trilha-de-aprendizagem.png?raw=true)

---

## 📄 Informações do Portal

**Versão da disciplina:** 2026.1

**Última atualização:** Junho de 2026

**Framework:** Fatec Academic Framework

